...
class CollisionAnalysisWindow(QMainWindow):
    ...
    def calculate_all_maneuvers(self):
        """Открыть полноценный калькулятор безопасного расхождения"""
        try:
            from safe_passing_dialog import SafePassingDialog
            ...
            # Открываем полноценный диалог калькулятора
    ...
    def show_maneuver_results(self, maneuvers, ships):
        """Показать результаты расчета маневров"""
    ...
    def apply_maneuvers(self, maneuvers, ships):
        """Применить рассчитанные маневры к судам"""
        for ship in ships:
            maneuver = maneuvers.get(ship.name)
            if maneuver and maneuver.get('is_valid', False) and maneuver.get('turn_angle_deg', 0) != 0:
                ...
                # Устанавливаем руль для поворота
    ...
    def update_table(self):
        ...
        for i in range(len(ships)):
            for j in range(i + 1, len(ships)):
                ...
                from units import format_distance, format_speed

                dist_item = QTableWidgetItem(format_distance(cpa_data['dist'], self.parent().use_miles if hasattr(self.parent(), 'use_miles') else True))
                self.table.setItem(row, 1, dist_item)
                
                cpa_item = QTableWidgetItem(format_distance(cpa_data['DCPA'], self.parent().use_miles if hasattr(self.parent(), 'use_miles') else True))
